...
This list shows every package installed in the compute environment:
    ...
    ap22-auth-mysql-1.11.12nb10 Module to allow Apache 2 authentication against a MySQL database
    ...
    font-bh-ttf-1.0.3nb1 Standard Bigelow & Holmes TrueType fonts
    ...
    gocr-0.47nb7         GOCR is a set of OCR tools
    ...
    ispell-svenska-1.2.1 Swedish dictionary for interactive spelling checker
    ...
    libgcrypt-1.5.0      GNU cryptographic library
    ...
    lua-filesystem-1.5.0 File system related library for Lua
    ...
    net-snmp-5.7.2       Extensible SNMP implementation
    ...
    p5-Class-Load-XS-0.06 XS implementation of parts of Class::Load
    ...
    p5-Date-Manip-6.34   Perl5 module for date calculations
    ...
    p5-Getopt-Long-2.38nb3 Extended processing of command line options
    ...
    p5-Net-Netmask-1.9016nb1 Perl5 module to parse and manipulate IP network blocks
    ...
    p5-Sub-Exporter-Progressive-0.001006nb1 Only use Sub::Exporter if you need it
    ...
    p5-URI-1.60nb1       Perl5 Uniform Resource Identifiers class (URI, RFC 2396)
    ...
    percona-toolkit-2.1.7nb1 Advanced command-line tools for open-source databases (MySQL)
    ...
    php53-pgsql-5.3.23   PHP extension for PostgreSQL databases
    ...
    php54-pdo_pgsql-5.4.13 PHP extension for PHP Data Objects (PostgreSQL)
    ...
    postgresql91-datatypes-9.1.9 PostgreSQL data types support modules
    ...
    py26-mongo-2.4.2     Python driver for MongoDB
    ...
    py27-libxml2-2.9.0   Python wrapper for libxml2
    py27-matplotlib-1.2.0nb1 Matlab-style plotting package for Python
    py27-memcached-1.48  Python memcached client
    py27-mongo-2.4.2     Python driver for MongoDB
    py27-mxDateTime-3.2.4 Python extension that provides types and conversions for dates
    py27-mysqldb-1.2.3nb3 MySQL interface for Python
    py27-nose-1.1.2      Unittest-based testing framework for python
    py27-numpy-1.6.2nb1  Array processing for numbers, strings, records, and objects
    py27-oauth-1.0.1     Python OAuth library
    py27-openid-2.2.5    Python module for OpenID
    py27-parsing-1.5.2   General parsing module for Python
    py27-pexpect-2.3nb1  Pure Python Expect-like module
    py27-pip-1.2.1       Installs Python packages as an easy_install replacement
    py27-postgresql-4.1.1 Python interface to PostgreSQL
    py27-psycopg2-2.4.6  PostgreSQL database adapter for Python
    py27-pytz-2012h      World timezone definitions, modern and historical
    py27-readline-0nb5   Line editing support for Python
    py27-reportlab-2.5nb2 Powerful PDF-generating toolkit for Python
    py27-roman-1.4.0     Integer to Roman numerals converter for Python
    py27-scipy-0.7.2nb1  Scientific Algorithms Library for Python
    py27-setuptools-0.6c11nb1 New Python packaging system
    py27-simplejson-2.6.2 Simplejson is a simple, fast, extensible JSON encoder/decoder
    py27-sqlite2-2.6.3nb4 SQLite database adapter for Python
    py27-sqlite3-0nb5    Built-in sqlite support for Python 2.5 and up
    py27-swish-e-0.5nb2  Simple Python binding for Swish-E
    py27-twisted-12.3.0nb1 Framework for writing networked applications
    ...
    py27-xmpppy-0.5.0rc1nb2 XMPP (Jabber) library for Python
    py27-yaml-3.10nb1    Collection of libraries to process YAML with Python
    py31-OpenSSL-0.13nb1 Python interface to the OpenSSL library
    py31-Tk-0nb6         Tkinter -- Python interface to Tcl/Tk
    py31-ZopeInterface-3.8.0 The zope.interface package used in Zope 3 and Twisted
    py31-cElementTree-0nb1 C implementation of Python ElementTree API
    py31-crypto-2.6nb1   Cryptographic and hash functions for Python
    py31-distribute-0.6.35 Replacement for setuptools
    py31-docutils-0.9.1  Python tool to generate documents
    py31-expat-0nb6      Python interface to expat
    py31-nose-1.1.2      Unittest-based testing framework for python
    py31-numpy-1.6.2nb1  Array processing for numbers, strings, records, and objects
    py31-psycopg2-2.4.6  PostgreSQL database adapter for Python
    py31-pytz-2012h      World timezone definitions, modern and historical
    py31-readline-0nb5   Line editing support for Python
    py31-roman-1.4.0     Integer to Roman numerals converter for Python
    py31-sqlite3-0nb5    Built-in sqlite support for Python 2.5 and up
    py31-yaml-3.10nb1    Collection of libraries to process YAML with Python
    py32-OpenSSL-0.13nb1 Python interface to the OpenSSL library
    py32-Tk-0nb6         Tkinter -- Python interface to Tcl/Tk
    py32-ZopeInterface-3.8.0 The zope.interface package used in Zope 3 and Twisted
    py32-cElementTree-0nb1 C implementation of Python ElementTree API
    py32-crypto-2.6nb1   Cryptographic and hash functions for Python
    py32-distribute-0.6.35 Replacement for setuptools
    py32-docutils-0.9.1  Python tool to generate documents
    py32-expat-0nb6      Python interface to expat
    py32-nose-1.1.2      Unittest-based testing framework for python
    py32-numpy-1.6.2nb1  Array processing for numbers, strings, records, and objects
    py32-psycopg2-2.4.6  PostgreSQL database adapter for Python
    py32-pytz-2012h      World timezone definitions, modern and historical
    py32-readline-0nb5   Line editing support for Python
    py32-roman-1.4.0     Integer to Roman numerals converter for Python
    py32-sqlite3-0nb5    Built-in sqlite support for Python 2.5 and up
    py32-yaml-3.10nb1    Collection of libraries to process YAML with Python
    py33-OpenSSL-0.13nb1 Python interface to the OpenSSL library
    py33-Tk-0nb6         Tkinter -- Python interface to Tcl/Tk
    py33-ZopeInterface-3.8.0 The zope.interface package used in Zope 3 and Twisted
    py33-cElementTree-0nb1 C implementation of Python ElementTree API
    py33-crypto-2.6nb1   Cryptographic and hash functions for Python
    py33-distribute-0.6.35 Replacement for setuptools
    py33-docutils-0.9.1  Python tool to generate documents
    py33-expat-0nb6      Python interface to expat
    py33-nose-1.1.2      Unittest-based testing framework for python
    py33-psycopg2-2.4.6  PostgreSQL database adapter for Python
    py33-pytz-2012h      World timezone definitions, modern and historical
    py33-readline-0nb5   Line editing support for Python
    py33-roman-1.4.0     Integer to Roman numerals converter for Python
    py33-sqlite3-0nb5    Built-in sqlite support for Python 2.5 and up
    py33-yaml-3.10nb1    Collection of libraries to process YAML with Python
    python26-2.6.8nb3    Interpreted, interactive, object-oriented programming language
    python27-2.7.3nb3    Interpreted, interactive, object-oriented programming language
    python31-3.1.5nb2    Interpreted, interactive, object-oriented programming language
    python32-3.2.3nb2    Interpreted, interactive, object-oriented programming language
    python33-3.3.0nb2    Interpreted, interactive, object-oriented programming language
    pyzor-0.5.0          Collaborative, networked system to detect and block spam
    qhull-2003.1nb1      Computing the convex hull
    qpress-1.1           qpress file archiver
    qt4-libs-4.8.4nb3    C++ X GUI toolkit
    qt4-tools-4.8.4nb2   QT GUI (WYSIWYG) builder and other tools
    quickbackup-mongodb-2.7nb20130502 MongoDB quickbackup SMF service
    quickbackup-percona-3.5nb20130502 Percona quickbackup SMF service
    rabbitmq-2.8.2       Implementation of AMQP that provides robust messaging for applications
    rabbitmq-c-0.3.0     C-language AMQP client library for use with AMQP servers
    randrproto-1.4.0     Randr extension headers from modular X.org
    rbldnsd-0.996b       Efficient DNS daemon to serve DNSBL zones
    ...
    recordproto-1.14.2   Record extension headers from X.org
    redis-2.6.9          Persistent key-value database with built-in net interface
    ree-2012.02          Server-oriented friendly branch of Ruby with various enhancements
    rename-1.3nb2        Rename recursively with regex patterns
    renderproto-0.11.1   Render extension headers from modular X.org
    resourceproto-1.2.0  Resource extension headers from X.org
    revbump-2.4          Tools for bumping PKGREVISIONs (for pkgsrc developers)
    riak-1.3.1           Distributed, highly available data store
    roundcube-0.8.6      Browser-based multilingual IMAP client
    rpm-2.5.4nb8         The Red Hat Package Manager
    rrdtool-1.4.7nb9     Data analysis tool generating graphical representations
    rsync-3.0.9nb1       Network file distribution/synchronisation utility
    rsyslog-6.2.0        The enhanced syslogd for Unix
    rtorrent-0.9.2nb2    Ncurses based torrent client with support for sessions
    ruby18-1.8.7.371     Ruby programming language 1.8.7 meta package
    ruby18-RMagick-2.13.2nb1 Ruby binding to ImageMagick
    ruby18-ZenTest-4.9.0 Testing utilities for Ruby
    ruby18-actionmailer-3.2.13 Email composition, delivery, and receiving (for Rails 3.2)
    ruby18-actionpack-3.2.13 Toolkit for building modeling frameworks (part of Rails 3.2)
    ruby18-activemerchant-1.20.0 Ruby library for credit card processing
    ruby18-activemodel-3.2.13 Toolkit for building modeling frameworks (part of Rails 3.2)
    ruby18-activerecord-3.2.13 Object-relational mapper framework (part of Rails 3.2)
    ruby18-activeresource-3.2.13 REST modeling framework (part of Rails 3.2)
    ruby18-activesupport-3.2.13 Toolkit of support libraries (part of Rails 3.2)
    ruby18-addressable-2.3.3 Replacement for the URI Implementation of standard librrary
    ruby18-amq-client-0.9.12 Fully-featured, low-level AMQP 0.9.1 client
    ruby18-amq-protocol-1.2.0 AMQP 0.9.1 encoder & decoder
    ruby18-amqp-0.9.10   AMQP client implementation in Ruby/EventMachine
    ruby18-arel-3.0.2    Arel is a SQL AST manager for Ruby
    ruby18-base-1.8.7.371nb1 Ruby 1.8 based release minimum package
    ruby18-bcrypt-3.0.1  OpenBSD's bcrypt() password hashing algorithm for Ruby
    ruby18-bluecloth-2.2.0 Markdown library for Ruby
    ruby18-borges-1.1.0nb2 Ruby web application framework
    ruby18-builder-3.0.4 Programatic generation of XML markup in Ruby
    ruby18-bundler-1.3.4 Manage your application's dependencies
    ruby18-bunny-0.6.0   Synchronous Ruby AMQP client
    ruby18-camping-2.1.531 Ruby micro-framework for web applications
    ruby18-capistrano-2.14.2 Tool for parallel execution of commands across multiple machines
    ruby18-coderay-1.0.9 Syntax highlighter for Ruby and other languages
    ruby18-columnize-0.3.6 Ruby module to print an Array in column-sorted order
    ruby18-curses-1.8.7.371 Ruby extension to curses library
    ruby18-daemon_controller-1.1.2 Library for robust daemon management
    ruby18-daemons-1.1.9 Toolkit to convert Ruby scripts to a daemon
    ruby18-data_objects-0.10.12 DataObjects basic API and shared driver specifications
    ruby18-datamapper-1.2.0 Object/Relational Mapper for Ruby
    ruby18-debug-0.10.4nb3 Command line interface for ruby-debug
    ruby18-debug-base-0.10.4nb1 Fast implementation of the standard Ruby debugger
    ruby18-diff-lcs-1.2.1 Diff library for Ruby
    ruby18-dm-aggregates-1.2.0 DataMapper plugin for aggregates on collections
    ruby18-dm-constraints-1.2.0 DataMapper plugin constraining relationships
    ruby18-dm-core-1.2.0nb1 Object/Relational Mapper for Ruby
    ruby18-dm-do-adapter-1.2.0 DataObjects Adapter for DataMapper
    ruby18-dm-migrations-1.2.0 DataMapper plugin for writing and speccing migrations
    ruby18-dm-mysql-adapter-1.2.0nb2 MySQL Adapter for DataMapper
    ruby18-dm-postgres-adapter-1.2.0 PostgreSQL Adapter for DataMapper
    ruby18-dm-serializer-1.2.2 DataMapper plugin for serializing Resources and Collections
    ruby18-dm-sqlite-adapter-1.2.0 Sqlite3 Adapter for DataMapper
    ruby18-dm-sweatshop-1.2.0 DataMapper plugin for building pseudo random models
    ruby18-dm-timestamps-1.2.0 DataMapper plugin for magical timestamps
    ruby18-dm-transactions-1.2.0 Adds transaction support to datamapper
    ruby18-dm-types-1.2.2 DataMapper plugin providing extra data types
    ruby18-dm-validations-1.2.0 Library for validations on DM models and pure Ruby object
    ruby18-dnssd-2.0     DNS Service Discovery (aka Bonjour, MDNS) API for Ruby
    ruby18-do_mysql-0.10.12nb1 MySQL driver for DataObjects
    ruby18-do_postgres-0.10.12 PostgreSQL driver for DataObjects
    ruby18-do_sqlite3-0.10.12 Implements the DataObjects API for Sqlite3
    ruby18-domain_name-0.5.8 Domain name manipulation library for Ruby
    ruby18-erubis-2.7.0  Fast and extensible eRuby implementation which supports multi-language
    ruby18-eruby-1.0.5nb8 Interprets Ruby code embedded in a text file
    ruby18-eventmachine-1.0.3 Ruby event-driven I/O using the Reactor pattern
    ruby18-facter-1.6.18 Cross-platform Ruby library for retrieving facts from OS
    ruby18-fastercsv-1.5.5 Replacement to Ruby standard CSV library
    ruby18-fcgi-0.8.8nb1 FastCGI interface for Ruby
    ruby18-ferret-0.11.8.5 Text search engine library written for Ruby
    ruby18-flexmock-1.3.1 Flexible mock object library for Ruby unit testing
    ruby18-gdbm-1.8.7.371nb2 Ruby extension to GDBM library
    ruby18-gem_plugin-0.2.3 Gem-based plugin system
    ruby18-gemcutter-0.7.1 Commands to interact with RubyGems.org
    ruby18-heckle-1.4.3nb2 Code mutation tester for Ruby
    ruby18-heel-3.0.2    Static file web server based on Rack and Thin
    ruby18-highline-1.6.16 High-level console IO library
    ruby18-hike-1.2.1    Find files in a set of paths
    ruby18-hoe-3.5.1     Rake extension to provide full project automation
    ruby18-hpricot-0.8.6 Fast, enjoyable HTML parser for Ruby
    ruby18-httpclient-2.1.5.2 Like libwww-perl for Ruby
    ruby18-i18n-0.6.4    New wave Internationalization support for Ruby
    ruby18-inline-3.12.1 Embed C/C++ code directly in Ruby
    ruby18-innate-2013.02.21 Powerful web-framework wrapper for Rack
    ruby18-journey-1.0.4 Journey is a router
    ruby18-json-1.7.7    Native extension implementation of JSON for Ruby
    ruby18-json-pure-1.7.7 Pure Ruby implementation of JSON
    ruby18-kgio-2.8.0    Provides non-blocking I/O methods for Ruby
    ruby18-launchy-2.2.0nb1 Ruby class for launching applications
    ruby18-libxml-2.6.0  Support module for libxml2 library
    ruby18-linecache-0.46 Ruby module to read and cache lines of a file
    ruby18-little-plugger-1.1.3 Module that provides Gem based plugin management
    ruby18-lockfile-2.1.0 Ruby library for creating NFS safe lockfiles
    ruby18-log4r-1.1.10  Flexible logging library
    ruby18-logging-1.8.1 Flexible logging library for Ruby based on log4j
    ruby18-mab-0.0.3     Markup as Ruby, write HTML in plain Ruby
    ruby18-mail-2.5.3    Mail provides a nice Ruby DSL for making, sending and reading emails
    ruby18-markaby-0.7.2 HTML Markup as Ruby
    ruby18-mechanize-2.5.1 Library to automate interaction with websites
    ruby18-memcache-client-1.7.4 A ruby client for Danga Interactive's memcached
    ruby18-metaclass-0.0.1 Adds a metaclass method to all Ruby objects
    ruby18-metaid-1.0nb1 Ruby metaprogramming helpers
    ruby18-mime-types-1.21 Definition of MIME types for Ruby
    ruby18-mini-magick-3.5.0 Ruby wrapper for ImageMagick command line
    ruby18-mocha-0.13.3  Ruby library for mocking and stubbing
    ruby18-mongrel-1.1.5nb3 Fast HTTP library and server for Ruby
    ruby18-mongrel-cluster-1.0.5nb1 Mongrel plugin to manage a mongrel cluster
    ruby18-mongrel-config-0.3.1nb1 Mongrel web-based configuration tool
    ruby18-mongrel-console-0.2.1nb1 Mongrel & Rails IRB console
    ruby18-mongrel-upload-progress-0.2.2nb1 Mongrel plugin to track file upload status
    ruby18-moqueue-0.1.4 Mocktacular Companion to AMQP Library
    ruby18-multi_json-1.7.0 General-purpose swappable JSON backend library
    ruby18-mysql-2.8.2nb4 Ruby extension for MySQL
    ruby18-narray-0.6.0.8 N-dimensional Numerical Array class for Ruby
    ruby18-needle-1.3.0  Dependency injection for Ruby
    ruby18-net-http-digest_auth-1.2.1 Implementation of RFC 2617 - Digest Access Authentication
    ruby18-net-http-persistent-2.8 Manages persistent connections using Net::HTTP
    ruby18-net-scp-1.1.0 Secure copy for Ruby
    ruby18-net-sftp-2.1.1 Secure FTP for Ruby
    ruby18-net-ssh-2.6.5 Secure shell for Ruby
    ruby18-net-ssh-gateway-1.2.0 Simple way to connect to servers behind a firewall or NAT
    ruby18-nokogiri-1.5.6 HTML, XML, SAX, and Reader parser with XPath and CSS selector support
    ruby18-ntlm-http-0.1.1 NTLM authentication over HTTP
    ruby18-oniguruma-1.1.0nb2 Fast regular expression library
    ruby18-parser-3.1.1  Ruby parser written in pure ruby
    ruby18-parsetree-3.0.9 Extract and enumerate Ruby parse trees
    ruby18-passenger-3.0.17 Modules for running Ruby on Rails and Rack applications
    ruby18-patron-0.4.18nb2 Ruby HTTP client library based on libcurl
    ruby18-polyglot-0.3.3 Registry of file types to load with its improved version of 'require'
    ruby18-puma-1.6.3    Modern concurrent web server for Ruby
    ruby18-puppet-0.25.5nb1 Configuration management framework written in Ruby
    ruby18-rack-1.5.2    Modular Ruby webserver interface
    ruby18-rack-cache-1.2nb1 HTTP Caching for Rack
    ruby18-rack-ssl-1.3.3nb1 Rack middleware to force SSL/TLS
    ruby18-rack-test-0.6.2nb1 Simple testing API built on Rack
    ruby18-rack14-1.4.5  Modular Ruby webserver interface
    ruby18-rails-3.2.13  Full-stack web application framework, Rails 3.2
    ruby18-railties-3.2.13 Tools for creating, working with, and running Rails 3.2
    ruby18-raindrops-0.10.0 Real-time statistics for Rack HTTP servers
    ruby18-rake-10.0.3   Ruby Make
    ruby18-ramaze-2012.12.08 Ruby lightweight, modular, web application framework
    ruby18-randexp-0.1.7 Library for generating random strings
    ruby18-rbx-require-relative-0.0.9 Ruby 1.9's require_relative for Rubinius and MRI 1.8
    ruby18-rcov-1.0.0    Code coverage analysis for Ruby
    ruby18-rcsparse-0.0.0.45nb1 RCS parsing library for Ruby
    ruby18-rdoc-3.12.2   produces HTML and command-line documentation for Ruby projects
    ruby18-rdtool-0.6.38 RD (Ruby Document) converter to HTML/man/etc
    ruby18-readline-1.8.7.371nb2 Ruby extension to readline library
    ruby18-redcloth-4.2.9 Textile library for Ruby
    ruby18-rest-client-1.6.7 Simple HTTP and REST client for Ruby inspired by Sinatra
    ruby18-rspec-2.13.0  Behaviour Driven Development framework for Ruby
    ruby18-rspec-core-2.13.1 Behaviour Driven Development framework for Ruby, core part
    ruby18-rspec-expectations-2.13.0 Behaviour Driven Development framework for Ruby, expectations
    ruby18-rspec-mocks-2.13.0 This is test-double framework for rspec
    ruby18-ruby2ruby-2.0.3 Pure Ruby code from Ruby S-expressions
    ruby18-rubygems-1.8.25 Ruby standard for publishing and managing third party libraries
    ruby18-sequel-3.45.0 Model classes for the Sequel Database Toolkit
    ruby18-sexp-processor-4.1.5 Generic sexp processing tools
    ruby18-soap4r-1.5.8  Implementation of SOAP 1.1 for Ruby
    ruby18-sprockets-2.2.2nb1 Rack-based asset packaging system
    ruby18-sqlite3-1.3.7nb1 Ruby interface for the SQLite database engine
    ruby18-stringex-1.5.1 Some (hopefully) useful extensions to Rubys String class
    ruby18-subexec-0.2.2 Subexec spawns an external command with a timeout
    ruby18-subversion-1.7.8 Ruby bindings for Subversion
    ruby18-syntax-1.0.0nb1 Ruby lexical analysis framework
    ruby18-taglib-0.5.2  Ruby library for reading and writing meta-data of many audio formats
    ruby18-term-ansicolor-1.0.4 Small Ruby library that colors strings using ANSI escape sequences
    ruby18-thin-1.5.0    Fast and very simple Ruby web server
    ruby18-thor-0.17.0   Scripting framework that replaces rake, sake and rubigen
    ruby18-tilt-1.3.5    Generic interface to multiple Ruby template engines
    ruby18-tk-1.8.7.371nb1 Ruby interface to the Tk widget set
    ruby18-treetop-1.4.12 Ruby-based text parsing and interpretation DSL
    ruby18-tzinfo-0.3.33 Daylight-savings-aware timezone library
    ruby18-unf-0.0.5     Wrapper library to bring Unicode Normalization Form support
    ruby18-unf_ext-0.0.6 Unicode Normalization Form support library for CRuby
    ruby18-unicorn-4.6.2 Rack HTTP server for fast clients and Unix
    ruby18-uuidtools-2.1.3 Simple universally unique ID generation library
    ruby18-validatable-1.6.7 Library for adding database/object validations
    ruby18-webrat-0.7.0  Ruby Acceptance Testing for Web applications
    ruby18-webrobots-0.1.1 Ruby library to help write robots.txt compliant web robots
    ruby18-xmpp4r-0.5    XMPP/Jabber library for Ruby
    ruby18-yajl-1.1.0    Ruby C binding to YAJL JSON library
    ruby193-1.9.3p392    Ruby programming language 1.9.3 meta package
    ruby193-RMagick-2.13.2nb1 Ruby binding to ImageMagick
    ruby193-actionmailer-3.2.13 Email composition, delivery, and receiving (for Rails 3.2)
    ruby193-actionpack-3.2.13 Toolkit for building modeling frameworks (part of Rails 3.2)
    ruby193-activemerchant-1.20.0 Ruby library for credit card processing
    ruby193-activemodel-3.2.13 Toolkit for building modeling frameworks (part of Rails 3.2)
    ruby193-activerecord-3.2.13 Object-relational mapper framework (part of Rails 3.2)
    ruby193-activeresource-3.2.13 REST modeling framework (part of Rails 3.2)
    ruby193-activesupport-3.2.13 Toolkit of support libraries (part of Rails 3.2)
    ruby193-addressable-2.3.3 Replacement for the URI Implementation of standard librrary
    ruby193-amq-client-0.9.12 Fully-featured, low-level AMQP 0.9.1 client
    ruby193-amq-protocol-1.2.0 AMQP 0.9.1 encoder & decoder
    ruby193-amqp-0.9.10  AMQP client implementation in Ruby/EventMachine
    ruby193-arel-3.0.2   Arel is a SQL AST manager for Ruby
    ruby193-base-1.9.3p392 Ruby 1.9.3 release minimum base package
    ruby193-bcrypt-3.0.1 OpenBSD's bcrypt() password hashing algorithm for Ruby
    ruby193-borges-1.1.0nb2 Ruby web application framework
    ruby193-builder-3.0.4 Programatic generation of XML markup in Ruby
    ruby193-bunny-0.6.0  Synchronous Ruby AMQP client
    ruby193-columnize-0.3.6 Ruby module to print an Array in column-sorted order
    ruby193-curses-1.9.3p392 Ruby extension to curses library
    ruby193-daemon_controller-1.1.2 Library for robust daemon management
    ruby193-daemons-1.1.9 Toolkit to convert Ruby scripts to a daemon
    ruby193-data_objects-0.10.12 DataObjects basic API and shared driver specifications
    ruby193-datamapper-1.2.0 Object/Relational Mapper for Ruby
    ruby193-diff-lcs-1.2.1 Diff library for Ruby
    ruby193-dm-aggregates-1.2.0 DataMapper plugin for aggregates on collections
    ruby193-dm-constraints-1.2.0 DataMapper plugin constraining relationships
    ruby193-dm-core-1.2.0nb1 Object/Relational Mapper for Ruby
    ruby193-dm-do-adapter-1.2.0 DataObjects Adapter for DataMapper
    ruby193-dm-migrations-1.2.0 DataMapper plugin for writing and speccing migrations
    ruby193-dm-mysql-adapter-1.2.0nb2 MySQL Adapter for DataMapper
    ruby193-dm-postgres-adapter-1.2.0 PostgreSQL Adapter for DataMapper
    ruby193-dm-serializer-1.2.2 DataMapper plugin for serializing Resources and Collections
    ruby193-dm-sqlite-adapter-1.2.0 Sqlite3 Adapter for DataMapper
    ruby193-dm-sweatshop-1.2.0 DataMapper plugin for building pseudo random models
    ruby193-dm-timestamps-1.2.0 DataMapper plugin for magical timestamps
    ruby193-dm-transactions-1.2.0 Adds transaction support to datamapper
    ruby193-dm-types-1.2.2 DataMapper plugin providing extra data types
    ruby193-dm-validations-1.2.0 Library for validations on DM models and pure Ruby object
    ruby193-dnssd-2.0    DNS Service Discovery (aka Bonjour, MDNS) API for Ruby
    ruby193-do_mysql-0.10.12nb1 MySQL driver for DataObjects
    ruby193-do_postgres-0.10.12 PostgreSQL driver for DataObjects
    ruby193-do_sqlite3-0.10.12 Implements the DataObjects API for Sqlite3
    ruby193-domain_name-0.5.8 Domain name manipulation library for Ruby
    ruby193-eventmachine-1.0.3 Ruby event-driven I/O using the Reactor pattern
    ruby193-fastercsv-1.5.5 Replacement to Ruby standard CSV library
    ruby193-fcgi-0.8.8nb1 FastCGI interface for Ruby
    ruby193-ferret-0.11.8.5 Text search engine library written for Ruby
    ruby193-fiddle-1.9.3p392 Ruby fiddle library to wrap libffi
    ruby193-flexmock-1.3.1 Flexible mock object library for Ruby unit testing
    ruby193-gdbm-1.9.3p392nb2 Ruby extension to GDBM library
    ruby193-gemcutter-0.7.1 Commands to interact with RubyGems.org
    ruby193-heckle-1.4.3nb2 Code mutation tester for Ruby
    ruby193-highline-1.6.16 High-level console IO library
    ruby193-hike-1.2.1   Find files in a set of paths
    ruby193-hpricot-0.8.6 Fast, enjoyable HTML parser for Ruby
    ruby193-httpclient-2.1.5.2 Like libwww-perl for Ruby
    ruby193-i18n-0.6.4   New wave Internationalization support for Ruby
    ruby193-inline-3.12.1 Embed C/C++ code directly in Ruby
    ruby193-innate-2013.02.21 Powerful web-framework wrapper for Rack
    ruby193-journey-1.0.4 Journey is a router
    ruby193-json-1.7.7   Native extension implementation of JSON for Ruby
    ruby193-json-pure-1.7.7 Pure Ruby implementation of JSON
    ruby193-kgio-2.8.0   Provides non-blocking I/O methods for Ruby
    ruby193-launchy-2.2.0nb1 Ruby class for launching applications
    ruby193-libxml-2.6.0 Support module for libxml2 library
    ruby193-little-plugger-1.1.3 Module that provides Gem based plugin management
    ruby193-lockfile-2.1.0 Ruby library for creating NFS safe lockfiles
    ruby193-log4r-1.1.10 Flexible logging library
    ruby193-logging-1.8.1 Flexible logging library for Ruby based on log4j
    ruby193-mab-0.0.3    Markup as Ruby, write HTML in plain Ruby
    ruby193-mail-2.5.3   Mail provides a nice Ruby DSL for making, sending and reading emails
    ruby193-markaby-0.7.2 HTML Markup as Ruby
    ruby193-mechanize-2.5.1 Library to automate interaction with websites
    ruby193-memcache-client-1.7.4 A ruby client for Danga Interactive's memcached
    ruby193-metaclass-0.0.1 Adds a metaclass method to all Ruby objects
    ruby193-metaid-1.0nb1 Ruby metaprogramming helpers
    ruby193-mime-types-1.21 Definition of MIME types for Ruby
    ruby193-mini-magick-3.5.0 Ruby wrapper for ImageMagick command line
    ruby193-mocha-0.13.3 Ruby library for mocking and stubbing
    ruby193-moqueue-0.1.4 Mocktacular Companion to AMQP Library
    ruby193-multi_json-1.7.0 General-purpose swappable JSON backend library
    ruby193-mysql-2.8.2nb4 Ruby extension for MySQL
    ruby193-narray-0.6.0.8 N-dimensional Numerical Array class for Ruby
    ruby193-needle-1.3.0 Dependency injection for Ruby
    ruby193-net-http-digest_auth-1.2.1 Implementation of RFC 2617 - Digest Access Authentication
    ruby193-net-http-persistent-2.8 Manages persistent connections using Net::HTTP
    ruby193-net-scp-1.1.0 Secure copy for Ruby
    ruby193-net-sftp-2.1.1 Secure FTP for Ruby
    ruby193-net-ssh-2.6.5 Secure shell for Ruby
    ruby193-net-ssh-gateway-1.2.0 Simple way to connect to servers behind a firewall or NAT
    ruby193-ntlm-http-0.1.1 NTLM authentication over HTTP
    ruby193-oniguruma-1.1.0nb2 Fast regular expression library
    ruby193-parser-3.1.1 Ruby parser written in pure ruby
    ruby193-parsetree-3.0.9 Extract and enumerate Ruby parse trees
    ruby193-passenger-3.0.17 Modules for running Ruby on Rails and Rack applications
    ruby193-patron-0.4.18nb2 Ruby HTTP client library based on libcurl
    ruby193-polyglot-0.3.3 Registry of file types to load with its improved version of 'require'
    ruby193-rack-1.5.2   Modular Ruby webserver interface
    ruby193-rack-cache-1.2nb1 HTTP Caching for Rack
    ruby193-rack-ssl-1.3.3nb1 Rack middleware to force SSL/TLS
    ruby193-rack-test-0.6.2nb1 Simple testing API built on Rack
    ruby193-rack14-1.4.5 Modular Ruby webserver interface
    ruby193-railties-3.2.13 Tools for creating, working with, and running Rails 3.2
    ruby193-raindrops-0.10.0 Real-time statistics for Rack HTTP servers
    ruby193-rake-10.0.3  Ruby Make
    ruby193-randexp-0.1.7 Library for generating random strings
    ruby193-rbtree-0.4.0 Sorted associative collection using Red-Black Trees for Ruby
    ruby193-rbx-require-relative-0.0.9 Ruby 1.9's require_relative for Rubinius and MRI 1.8
    ruby193-rcsparse-0.0.0.45nb1 RCS parsing library for Ruby
    ruby193-rdoc-3.12.2  produces HTML and command-line documentation for Ruby projects
    ruby193-readline-1.9.3p392nb2 Ruby extension to readline library
    ruby193-redcloth-4.2.9 Textile library for Ruby
    ruby193-rest-client-1.6.7 Simple HTTP and REST client for Ruby inspired by Sinatra
    ruby193-rspec-2.13.0 Behaviour Driven Development framework for Ruby
    ruby193-rspec-expectations-2.13.0 Behaviour Driven Development framework for Ruby, expectations
    ruby193-rspec-mocks-2.13.0 This is test-double framework for rspec
    ruby193-rubygems-1.8.25 Ruby standard for publishing and managing third party libraries
    ruby193-sexp-processor-4.1.5 Generic sexp processing tools
    ruby193-soap4r-1.5.8 Implementation of SOAP 1.1 for Ruby
    ruby193-sprockets-2.2.2nb1 Rack-based asset packaging system
    ruby193-sqlite3-1.3.7nb1 Ruby interface for the SQLite database engine
    ruby193-stringex-1.5.1 Some (hopefully) useful extensions to Rubys String class
    ruby193-subexec-0.2.2 Subexec spawns an external command with a timeout
    ruby193-syntax-1.0.0nb1 Ruby lexical analysis framework
    ruby193-taglib-0.5.2 Ruby library for reading and writing meta-data of many audio formats
    ruby193-term-ansicolor-1.0.4 Small Ruby library that colors strings using ANSI escape sequences
    ruby193-tilt-1.3.5   Generic interface to multiple Ruby template engines
    ruby193-tk-1.9.3p392nb1 Ruby interface to the Tk widget set
    ruby193-tzinfo-0.3.33 Daylight-savings-aware timezone library
    ruby193-unf-0.0.5    Wrapper library to bring Unicode Normalization Form support
    ruby193-unf_ext-0.0.6 Unicode Normalization Form support library for CRuby
    ruby193-uuidtools-2.1.3 Simple universally unique ID generation library
    ruby193-validatable-1.6.7 Library for adding database/object validations
    ruby193-webrat-0.7.0 Ruby Acceptance Testing for Web applications
    ruby193-webrobots-0.1.1 Ruby library to help write robots.txt compliant web robots
    ruby193-xmpp4r-0.5   XMPP/Jabber library for Ruby
    ruby193-yajl-1.1.0   Ruby C binding to YAJL JSON library
    rudiments-0.34       C++ class library for client/server/daemon applications
    samba-3.6.12nb2      SMB/CIFS protocol server suite
    scmgit-1.8.1.5       GIT version control suite meta-package
    scmgit-base-1.8.1.5  GIT Tree History Storage Tool (base package)
    scmgit-docs-1.8.1.5  GIT Tree History Storage Tool (documentation)
    scmgit-gitk-1.8.1.5  GIT Tree History Storage Tool (gitk)
    scons-2.2.0          Python-based, open-source build system
    scponly-4.8nb1       Shell that only permits sftp
    screen-4.0.3nb5      Multi-screen window manager
    scrnsaverproto-1.2.2 ScrnSaver extension headers from X.org
    scsh-0.6.7nb1        Unix shell embedded into Scheme, with access to all Posix calls
    see-3.0.1376nb5      JavaScript interpreter and runtime library in C
    shared-mime-info-1.1 Core database of common types
    shc-3.7              Shell script to C compiler
    shlock-20080529      Shell utility to perform dot-locking
    sid-milter-1.0.0nb2  Open source SPF/SenderID filter software from Sendmail, Inc
    siege-2.78           Website/webserver regression testing utility
    skey-1.1.5nb5        Port of OpenBSD s/key implementation
    smartdc-6.5.6        V8 JavaScript for clients and servers
    smtools-20130417     Joyent tools relevant to SmartOS and SmartMachines
    snappy-1.0.5         General purpose data compression library
    snort-2.8.5.1nb4     The Open Source Network Intrusion Detection System
    solaris-tap-20091116 Solaris kernel module for virtual Ethernet devices
    spamass-milter-0.3.1nb4 Milter interface to Spamassassin
    spamassassin-3.3.2nb3 Mail filter to identify spam
    spamprobe-1.4dnb7    Spam detector using Bayesian analysis of word counts
    spawn-fcgi-1.6.3nb3  FastCGI deployment tool
    spectrum-1.4.8nb19   XMPP transport/gateway based on libpurple
    speex-1.2rc1nb1      Open-source, patent-free voice codec
    sphinxsearch-2.0.6   Sphinx Full-Text Search Engine
    spice-protocol-0.12.3 Spice Protocol headers
    spidermonkey-1.8.0rc1nb4 Standalone JavaScript implementation in C
    sqlite-2.8.17nb1     SQL Database Engine in a C Library
    sqlite3-3.7.15.2nb2  SQL Database Engine in a C Library
    sqlrelay-0.43nb1     Enables pooling and sharing of database connections
    sqoop-1.4.2          Efficiently transfer bulk data between Hadoop and structured datastores
    squid-3.3.3nb1       Post-Harvest_cached WWW proxy cache and accelerator
    stud-0.3p53          Scalable TLS Unwrapping Daemon
    subversion-1.7.8     Version control system, meta-package
    subversion-base-1.7.8 Version control system, base programs and libraries
    sudo-1.7.10p7        Allow others to run commands as root
    sun-jdk6-6.0.26nb1   Sun's Java(tm) 2 Standard Edition,
    sun-jre6-6.0.26      Sun's Java(tm) 2 Standard Edition, Java Runtime Environment (JRE) 6u26
    swig-1.3.36nb2       Simplified Wrapper and Interface Generator (version 1)
    swish-e-2.4.5nb5     Simple web indexing for humans - enhanced
    sysbench-0.4.12nb3   Modular, cross-platform and multi-threaded benchmark tool
    sysexits-1.1         Substitute <sysexits.h> for OS's lacking it
    syslog-ng2-2.0.0     Syslog-ng embodies the next generation of logging systems
    t1lib-5.1.2nb6       Library for generating bitmaps from Adobe Type 1 fonts
    t1utils-1.37         Utility programs for dealing with Postscript Type 1 font files
    taglib-1.8nb1        Library for accessing id tags in various media files
    tcl-8.5.13           Tool Command Language, a dynamic language
    tcp_wrappers-7.6.4   Monitor and filter incoming requests for network services
    tcsh-6.18.01nb4      Extended C-shell with many useful features
    tdb-1.2.11           Small database system which uses files to store data
    teTeX-3.0nb11        Thomas Esser's TeX distribution for UNIX compatible systems
    teTeX-bin-3.0nb52    TeX distribution for UNIX compatible systems - binaries
    teTeX-texmf-3.0nb39  Machine-independent part of teTeX
    templeton-0.1.0      REST-like web API for Hadoop
    tex-2up-1.3          TeX package to print a document two-up
    tex-a2ping-2.77pnb1  Advanced PS, PDF, EPS converter
    ...
    tex-algorithms-2009  A suite of tools for typesetting algorithms in pseudo-code
    ...
    tex-beamer-3.10      A LaTeX class for producing presentations and slides
    ...
    tex-epstopdf-2.16nb1 Convert EPS to 'encapsulated' PDF using GhostScript
    ...
    tex-ifmtarg-1.2anb1  If-then-else command for processing potentially empty arguments
    ...
    tex-metapost-1.212   A development of MetaFont for creating graphics
    ...
    tex-minitoc-60       Produce a table of contents for each chapter, part or section
    ...
    tex-oberdiek-2012    Bundle of packages submitted by Heiko Oberdiek
    ...
    tex-pdfcrop-1.32nb1  Crop PDF graphics
    ...
    tex-pdftools-0.82nb1 PDF-related utilities, including PostScript-to-PDF conversion
    ...
    tex-pst-3d-1.10      A PSTricks package for tilting and other pseudo-3D tricks
    ...
    tex-pst-math-0.61    Enhancement of PostScript math operators to use with pstricks
    tex-pst-node-1.15    Draw connections using pstricks
    tex-pst-osci-2.82    Oscgons with PSTricks
    tex-pst-pdf-1.1vnb1  Make PDF versions of graphics by processing between runs
    ...
    tex-tetex-3.0.27774nb1 Supplementary files for updmap
    ...
    tex-texdoc-2011nb1   Documentation access for TeX distributions
    tex-texlive-scripts-2011 TeX Live infrastructure programs
    tex-texlive.infra-2012.27610 Basic TeX Live infrastructure
    ...
    texi2html-5.0nb2     Texinfo-to-HTML direct translator
    texlive-tetex-2011nb3 Scripts included in teTeX
    tftp-hpa-5.2         Enhanced version of the BSD TFTP client and server
    tidy-20091027nb4     Fixes and tidies up HTML files
    tiff-4.0.3nb2        Library and tools for reading and writing TIFF data files
    tk-8.5.13nb1         Graphical toolkit for TCL
    tmux-1.7nb1          BSD-licensed terminal multiplexer (GNU Screen alternative)
    tnftp-20070806       The enhanced FTP client in NetBSD
    tokyocabinet-1.4.46  Modern implementation of DBM
    tokyotyrant-1.1.41   Network interface of Tokyo Cabinet
    top-3.6.1nb1         The ubiquitous system monitoring utility
    trac-1.0             Repository browser, wiki, and issue tracking system
    transmission-2.77    Free, lightweight BitTorrent client
    trapproto-3.4.3      Trap extension headers
    ttcp-1.12nb2         TCP testing and performance measuring tool
    ...
    unison-2.40.102nb4   File-synchronization tool
    units-2.01           GNU Units - a calculator capable of performing unit conversions
    unixodbc-2.3.0nb2    ODBC 2.x/3.x driver manager
    unrar-4.2.4          Extract, view & test RAR archives
    unzip-6.0nb1         List, test and extract compressed files in a ZIP archive
    updating database: 100%
    updating database: 100%
    url2pkg-2.17         Tool to automate initial steps in building a package
    urw-fonts-1.0.7pre44 Standard postscript fonts (cyrillicized)
    usbids-20081118      Repository of USB IDs (usb.ids database)
    usbredir-0.4.4       USB redirection library
    varnish-3.0.2nb4     High-performace HTTP accelerator
    vera-ttf-1.10nb4     Bitstream Vera TrueType fonts
    verifypc-1.6         Sanity check package dependencies according to pkg-config
    videoproto-2.3.1     Video extension headers from modular X.org X11
    vim-7.3.762          Vim editor (vi clone) without GUI
    vim-share-7.3.762    Data files for the vim editor (vi clone)
    visitors-0.7         Very fast web log analyzer
    vorbis-tools-1.4.0nb3 Ogg Vorbis encoder and player
    vsftpd-2.3.4nb1      FTP server that aims to be very secure
    w3m-0.5.3nb8         Multilingualized version of a pager/text-based browser w3m
    watch-3.2.6nb1       Watch a program with update intervals
    web2c-2012nb6        TeX implementation translating WEB to C
    webalizer-2.21.2nb7  Web server log file analysis program
    wget-1.14nb2         Retrieve files from the 'net via HTTP and FTP
    wireshark-1.8.6      Network protocol analyzer
    wxGTK28-2.8.10nb23   GTK-based implementation of the wxWidgets GUI library
    x264-devel-20130115  GPL licensed H.264 encoder
    xauth-1.0.7          X authentication utility
    xcb-proto-1.8        XCB protocol descriptions (in XML)
    xcmiscproto-1.2.2    XCMisc extension headers from X.org
    xdvipdfmx-0.7.8.20120701nb1 Extended DVI to PDF converter, mainly for use with XeTeX
    xerces-c-3.1.1       Validating C++ XML parser with DOM and SAX support
    xetex-0.9998nb7      TeX system with Unicode and modern font technologies
    xextproto-7.2.1      XExt extension headers from X.org
    xf86bigfontproto-1.2.0 XF86BigFont extension headers from X.org
    xf86dgaproto-2.1     XF86DGA extension headers
    xf86driproto-2.1.1   XF86DRI extension headers from modular X.org
    xf86miscproto-0.9.3  XF86Misc extension headers from modular X.org
    xf86vidmodeproto-2.3.1 XF86VidMode extension headers from modular X.org
    xineramaproto-1.2.1  Xinerama extension headers from X.org
    xkbcomp-1.2.4        XKBD keymap compiler
    xkeyboard-config-2.8 X Keyboard Configuration Database
    xmlcatmgr-2.2nb1     XML and SGML catalog manager
    xmlrpc-c-ss-1.16.44nb2 Library for writing an XML-RPC server or client in C or C++
    xmlto-0.0.25nb1      Tool to help transform XML documents into other formats
    xorg-cf-files-1.0.4nb5 Xorg imake rules
    xorg-util-macros-1.17 Xorg autotool macros
    xproto-7.0.23nb2     X protocol and ancillary headers from Xorg X11
    xtrans-1.2.7         Network API translation layer to insulate X
    ...
    zookeeper-client-3.4.4 Highly reliable distributed coordination server (client)
